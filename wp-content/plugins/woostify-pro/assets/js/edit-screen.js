		...
		function( button ) {
			button.onclick = function() {
				...
				if ( options.length ) {
					options.forEach(
						function( el, index ) {
							if ( ! el.value.trim() && ! el.classList.contains( 'woostify-admin-color-picker' ) ) {
								return;
							}
				...
				fetch( request )
					.then(
						function( res ) {
							...
							if ( ! infoSuccess ) {
								button.insertAdjacentHTML( 'afterend', '<span class="info-seuccess">' + woostify_edit_screen.saved_success + '</span>' );
							}
